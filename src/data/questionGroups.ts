...
export const questionGroups: QuestionGroup[] = [
  // PROGRESIONES DIRECTAS (Números Romanos → Acordes)
...
export const getGroupsByCategory = (category: string) => {
  return questionGroups.filter(group => group.category === category);
};

export const getCategoryStats = () => {
  const stats = {
    'progression-direct': {
      groups: getGroupsByCategory('progression-direct').length,
      questions: getGroupsByCategory('progression-direct').reduce((sum, group) => sum + group.totalQuestions, 0)
    },
    'progression-inverse': {
      groups: getGroupsByCategory('progression-inverse').length,
      questions: getGroupsByCategory('progression-inverse').reduce((sum, group) => sum + group.totalQuestions, 0)
    },
    'scale-mode': {
      groups: getGroupsByCategory('scale-mode').length,
      questions: getGroupsByCategory('scale-mode').reduce((sum, group) => sum + group.totalQuestions, 0)
    }
  };
  
  const total = {
    groups: Object.values(stats).reduce((sum, cat) => sum + cat.groups, 0),
    questions: Object.values(stats).reduce((sum, cat) => sum + cat.questions, 0)
  };
